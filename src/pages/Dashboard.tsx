        ...
        <div className="container mx-auto px-4">
          ...
          <div className="mb-8">
            ...
            <h1 className="text-3xl md:text-4xl font-heading font-semibold mb-2">
              Welcome back, <span className="text-foreground">Learner</span>
            </h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              { icon: Trophy, label: "Global Rank", value: "#127", color: "text-foreground" },
              { icon: Flame, label: "Current Streak", value: "7 days", color: "text-orange-400" },
              { icon: Star, label: "Total Points", value: "2,450", color: "text-foreground" },
              { icon: Target, label: "Challenges Done", value: "12", color: "text-emerald-400" },
            ].map((stat, i) => (
              <div key={i} className="bg-card border border-border p-5 transition-all hover:border-foreground/40">
                ...
                {weeklyProgress.map((day, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-2">
                    <div className="w-full bg-muted rounded-t-lg relative overflow-hidden" style={{ height: `${Math.max(day.value, 5)}%` }}>
                      <div className="absolute inset-0 bg-foreground opacity-80" />
                    </div>
                ...
                {recentActivity.map((activity, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <div className={cn(
                      ...
                      activity.type === "challenge" && "bg-accent",
                      activity.type === "lesson" && "bg-accent",
                      activity.type === "streak" && "bg-orange-400/20"
                    )}>
                      {activity.type === "challenge" && <Target className="w-5 h-5 text-foreground" />}
                      {activity.type === "lesson" && <BookOpen className="w-5 h-5 text-foreground" />}
                    ...
                    {activity.points > 0 && (
                      <span className="text-sm font-medium text-foreground">+{activity.points}</span>
                    )}
                  </div>
                ))}
              </div>
